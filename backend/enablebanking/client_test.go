...
func TestTransactionsRequiresDateRange(t *testing.T) {
	client := testClient(t, func(http.ResponseWriter, *http.Request) {
		t.Fatal("request sent without a date range")
	})
	_, err := client.Transactions(t.Context(), "account", "", "2026-07-01", "")
	require.EqualError(t, err, "transaction date range is required")
}

func TestTransactionsSendsRangeAndContinuation(t *testing.T) {
	client := testClient(t, func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "2026-07-01", r.URL.Query().Get("date_from"))
		require.Equal(t, "2026-07-02", r.URL.Query().Get("date_to"))
		require.Equal(t, "next", r.URL.Query().Get("continuation_key"))
		_, _ = w.Write([]byte(`{"transactions":[{"status":"BOOK"}],"continuation_key":""}`))
	})
	page, err := client.Transactions(t.Context(), "account", "2026-07-01", "2026-07-02", "next")
	require.NoError(t, err)
...
func TestAPIError(t *testing.T) {
	...
	_, err := client.Transactions(t.Context(), "account", "2026-07-01", "2026-07-02", "")
