...
func (d *Data) AddPostingTag(ctx context.Context, userID string, postingIDs []string, tag string) (int, error) {
	...
	err := d.db.QueryRowContext(ctx, `
		with requested as (
			select distinct unnest($2::uuid[]) id
		), editable as (
			select p.id
			from requested r
			join postings p on p.id = r.id
		...
		select
			...
			(select count(*) from editable),
			(select count(*) from added)
	`, userID, postingIDs, tag).Scan(&requested, &editable, &added)
	if err != nil {
		return 0, err
	}
...
func (d *Data) RemovePostingTag(ctx context.Context, userID string, postingIDs []string, tag string) (int, error) {
	...
	err := d.db.QueryRowContext(ctx, `
		with requested as (
			...
			join editable e on e.id = pt.posting_id
			where pt.tag = $3
			  and (select count(*) from requested) = (select count(*) from editable)
		), audited as (
			insert into audit_logs
	...
	`, userID, postingIDs, tag).Scan(&requested, &editable, &removed)
	if err != nil {
		return 0, err
	}
	if requested != editable {
