	...
	return db.query<DirtyEntry>(
		...
		select * from (
			...
			select
				...
				_sync_is_deleted,
				_sync_edited_at,
				json_object('created_at', created_at, 'updated_at', updated_at, 'name', name, 'currency', currency, 'external_id', external_id, 'code', code) as plain_data,
				1 as priority
	...
	if (accounts.length) {
		await db.exec(
			`insert into accounts (
				id, created_at, updated_at, name, currency, external_id, code,
