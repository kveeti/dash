...
`.simple();
	...
	const scripts = [script_0, script_1, script_2, script_3];
	...
	await pg.begin(async (t) => {
		...
		if (!row) {
			await t`insert into __version (v) values (0);`;
		} else {
			version = row.v;
		}

		if (version == scripts.length - 1) {
			console.log("no migrations to run", { version });
			return;
		}

		let latestVersion = 0;
		for (let i = version === 0 ? version : version + 1; i < scripts.length; i++) {
			const script = scripts[i];
			await script(t);
			await t`update __version set v = ${i}`;
			latestVersion = i;
		}

		console.log("migrated db from", version, "to", latestVersion);
	});
}
