...
test.describe("group", async () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/combobox/group");
  });

  test("groups are shown/hidden based on item matches", async ({ page }) => {
    await page.locator(`[cmdk-input]`).type("z");
    await expect(
      page.locator(`[cmdk-group][data-value="Animals"]`),
    ).not.toBeVisible();
    await expect(
      page.locator(`[cmdk-group][data-value="Letters"]`),
    ).toBeVisible();
  });

  test("group can be progressively rendered", async ({ page }) => {
    await expect(
      page.locator(`[cmdk-group][data-value="Numbers"]`),
    ).not.toBeVisible();
  ...
  test("mounted group still rendered with filter using forceMount", async ({
    page,
  }) => {
