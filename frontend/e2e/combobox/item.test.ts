...
test.describe("item", async () => {
  ...
  test("unmount item that is selected", async ({ page }) => {
    await page.locator(`data-testid=mount`).click();
    await expect(page.locator(`[cmdk-item][aria-selected="true"]`)).toHaveText(
      "A",
    );
  ...
  test("mounted item still rendered with filter usingForceMount", async ({
    page,
  }) => {
...
test.describe("item advanced", async () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/combobox/item-advanced");
  });

  test("re-rendering re-matches implicit textContent value", async ({
    page,
  }) => {
