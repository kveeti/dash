// Solid port of cmdk. Adapted mainly from create-signal/cmdk-solid
// (https://github.com/create-signal/cmdk-solid), itself a port of the React
// original at https://github.com/dip/cmdk.
...
export type CommandListProps = Children &
  DivProps & {
     ...
     */
    label?: string;
    /** Hide the scrollbar while keeping the list scrollable. */
...
const Command: Component<CommandRootProps> = (props) => {
  const [state, setState] = createStore<State>({
    ...
    selectedItemId: "",
    filtered: { count: 0, items: {}, groups: [], order: {} },
    items: [],
    groups: {},
    ids: {},
  });
  ...
  createEffect(() => {
    ...
    // Visual sort via CSS `order` (see Item/Group/List). DOM order stays put so
    // Solid's reconciler never reorders <For> nodes. Items rank by score; groups
    // by their best item, offset so they trail loose items.
    const order: Record<string, number> = {};
    [...state.items]
      .sort((a, b) => (items[b] || 0) - (items[a] || 0))
      .forEach((id, i) => (order[id] = i));
    const groupMax = (groupId: string) =>
      state.groups[groupId]!.reduce(
        (max, id) => Math.max(max, items[id] || 0),
        0,
      );
    Object.keys(state.groups)
      .sort((a, b) => groupMax(b) - groupMax(a))
      .forEach((groupId, i) => (order[groupId] = 10000 + i));

    setState("filtered", { count, items, groups, order });
  });
    ...
    setState: (key, value, opts) => {
      ...
      if (key === "search") {
        ...
        schedule(7, () =>
          setState(
            "selectedItemId",
            getSelectedItem()?.getAttribute("id") || "",
          ),
        );
...
const Item: ParentComponent<CommandItemProps> = (props) => {
  ...
  const order = useCmdk((state) => state.filtered.order[id] ?? 0);
  ...
  return (
    <Show when={render()}>
      <div
        ...
        cmdk-item=""
        role="option"
        style={styleWithOrder(etc.style, order())}
...
const List: ParentComponent<CommandListProps> = (props) => {
  ...
  return (
    <div
      ...
      {...etc}
      cmdk-list=""
      data-hide-scrollbar={localProps.hideScrollbar ? "" : undefined}
      role="listbox"
      tabindex={-1}
      aria-activedescendant={selectedItemId() || undefined}
...
/** Merge an item/group's `order` into whatever `style` the caller passed. */
function styleWithOrder(style: unknown, order: number) {
  if (typeof style === "string") return `${style};order:${order}`;
  return { ...(style as JSX.CSSProperties), order };
}
